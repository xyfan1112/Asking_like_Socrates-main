...
def _parse_box_from_response_boxed_only(response: str) -> Optional[List[float]]:
    """
    Parse 4 normalized floats ONLY if \\boxed{...} appears AFTER the first </think>;
    otherwise return None (invalid).
    """
    inner = _extract_boxed_after_think(response)
    if inner is None:
        return None
    return _parse_four_floats_0_1(inner)

# ---- NEW: helpers enforcing 'boxed AFTER </think>' ----

def _extract_boxed_after_think(response: str) -> Optional[str]:
    """
    Return the content inside the FIRST \\boxed{...} that appears AFTER the first </think>.
    If </think> or boxed-after-think is absent, returns None.
    """
    think_close = re.search(r"</think>", response, flags=re.IGNORECASE | re.DOTALL)
    if not think_close:
        return None
    boxed_pat = re.compile(r"\\boxed\{\s*(.*?)\s*\}", flags=re.DOTALL)
    m = boxed_pat.search(response, think_close.end())
    return m.group(1) if m else None

def _has_valid_boxed_four_after_think(response: str) -> bool:
    inner = _extract_boxed_after_think(response)
    return inner is not None and (_parse_four_floats_0_1(inner) is not None)
...
def _unpack_gt_bundle(gt: Any, idx: int) -> Tuple[List[float], int, int, str]:
    ...
    # If reaches here, schema is invalid for the new reward design:
    try:
...
def _has_valid_boxed_four(response: str) -> bool:
    # kept for reference; not used in new scoring (we require AFTER </think>)
...
def format_reward(
    ...
    """
    New Format reward with an extra rule:
      - \\boxed{...} MUST appear AFTER the first </think> and contain 4 [0,1] floats.

    Instructions considered (binary completion):
      1) Presence of <think>...</think>
      2) Valid \\boxed AFTER </think>
      3) <think> opening matches expected_opening (prefix, case-insensitive)
      4) <think> word budget respected: words <= expected_words
      5) <think> 'wait' count equals expected_waits

    Returns fmt_raw = (e^m - 1) / (e^n - 1)
    """
    ...
    # (2) boxed-after-think is valid
    instr_flags.append(_has_valid_boxed_four_after_think(response))

    # Prepare think text for remaining checks
...
def thinking_reward(
    ...
    """Fine-grained thinking reward (unchanged)."""
...
def accuracy_reward_boxes(
    ...
    Combined accuracy using:
        IoU_score    = exp((IoU*5) - 1) / (e**5 - 1)   # NEW mapping
        center_score = r / max(D, r), with r = 0.5 * diagonal_length(gt)
        acc          = (1 - center_weight) * IoU_score + center_weight * center_score

    NOTE: Only parse pred bbox if \\boxed{...} is AFTER </think>. Otherwise (0.0, 0.0).
    """
    ...
    # New IoU -> score mapping
    denom = math.exp(5.0) - 1.0
    iou_score = (math.exp((iou * 5.0) - 1.0) / denom) if denom > 0 else 0.0
    iou_score = float(max(0.0, min(1.0, iou_score)))

    # Center score (unchanged)
...
def compute_score(
    ...
    thinking score is returned for diagnostics but not used in 'overall'.
    """
    ...
    for idx, item in enumerate(reward_inputs):
        ...
        # accuracy (returns 0 if pred missing / order wrong)
        acc, iou = accuracy_reward_boxes(response, v["gt_bbox"], center_weight=center_weight)

        # thinking control reward (diagnostic)
        ...
        results.append({
            "overall": overall,
            "accuracy_raw": acc,   # before subtracting E
            "accuracy": acc_adj,   # after subtracting E
            "format": fmt_raw,
            "thinking": thinking,
            "IOU": iou,
