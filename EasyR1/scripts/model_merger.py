...
# 方式一：优先尝试 save_pretrained（禁用 safe_serialization）
    try:
        ...
        print(f"[warn] save_pretrained failed ({e}), fallback to torch.save")
# 确保目录存在
        os.makedirs(hf_path, exist_ok=True)

# 使用 transformers 的 shard 文件保存逻辑
        from transformers.modeling_utils import shard_checkpoint, WEIGHTS_NAME, WEIGHTS_INDEX_NAME

# 每个分片最大 2GB，可以按需调整成 "1GB"、"5GB" 等
        max_shard_size = "2GB"
...
# 保存分片
        for shard_file, shard in shards.items():
            shard_file = os.path.join(hf_path, shard_file)
            torch.save(shard, shard_file)

# 保存索引文件
        index_file = os.path.join(hf_path, WEIGHTS_INDEX_NAME)
...
# 保存 config
        ...
        print(f"[info] Model sharded into {len(shards)} files (max shard size {max_shard_size}).")
