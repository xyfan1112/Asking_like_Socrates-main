...
[[ -d "$ACTIVE_DATASET_ROOT" ]] || fail "数据集目录不存在: $ACTIVE_DATASET_ROOT"
...
start_eval_replicas() {
  local key=$1
  require_cfg; read_eval_arrays; stop_eval_replicas
  stop_socratic_all
  local -a pids=()
  for ((i=0;i<${#EVAL_GPU_ARRAY[@]};i++)); do
...
safe_direct() { prepare; data; b0; b1; echo "[PASS] safe-direct完成；未启动Socratic。"; }
...
基础：
  ...
  prepare                只生成/检查当前语言独立配置；英文首次会生成类别翻译模板
...
case "${1:-}" in
  status) status ;; prepare) prepare ;; data|datafull) data ;; b0) b0 ;;
