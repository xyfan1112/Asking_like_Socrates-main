...
RS_REAL=$(readlink -f "$RS")
[[ "$HERE_REAL" == "$RS_REAL" ]] || fail "运行脚本与REPO_ROOT/RE_SCRIPTS_ROOT_OVERRIDE指向不同re_scripts：script_root=$HERE_REAL resolved_root=$RS_REAL。请取消错误override或修正commands/user_config.sh。"
RS="$RS_REAL"
echo "[RUNNER] release=$RELEASE_REVISION script=$SCRIPT_PATH root=$RS command=${1:-<none>}"
...
if errors:
    ...
    raise SystemExit(2)
print(f"[PASS] scene-disjoint manifest: train={counts.get('train')} val={counts.get('val')} overlap=0", file=sys.stderr)
PY
}
...
prepare() {
  ...
  cmd=("$BOOTSTRAP_PY" "$RS/tools/materialize_language_settings.py"
    ...
    --student-model-family "$STUDENT_MODEL_FAMILY"
    --student-base-model-path "$STUDENT_BASE_MODEL_PATH"
    --student-template "$STUDENT_MODEL_TEMPLATE"
    --student-b1-adapter-output "$STUDENT_B1_ADAPTER_OUTPUT"
    --student-b2-adapter-output "$STUDENT_B2_ADAPTER_OUTPUT"
    --student-adapter-only "$STUDENT_ADAPTER_ONLY"
    --student-freeze-vision "$STUDENT_FREEZE_VISION_TOWER"
    --student-freeze-projector "$STUDENT_FREEZE_PROJECTOR"
    --student-lora-target "$STUDENT_LORA_TARGET")
  ...
  "${cmd[@]}"
  # r3: downstream commands read taxonomy.classes_file directly from ACTIVE_CFG.
  # No Bash command substitution is used for paths, so progress logs cannot enter a path.
  "$BOOTSTRAP_PY" "$RS/tools/validate_active_taxonomy.py" \
    --settings "$ACTIVE_CFG" --expected-lang "$RUN_LANG"
  "$BOOTSTRAP_PY" "$RS/main_layer/run.py" check-taxonomy \
    --settings "$ACTIVE_CFG" --lang "$RUN_LANG"
...
if v.get('release_revision')!='r3': errors.append('release_revision')
...
if not s.get('model_routing',{}).get('b0_model_key'): errors.append('model_routing')
if errors: raise SystemExit('resolved settings mismatch: '+','.join(errors)+'. rerun prepare after removing old config')
PY
  "$BOOTSTRAP_PY" "$RS/tools/validate_active_taxonomy.py" \
    --settings "$ACTIVE_CFG" --expected-lang "$RUN_LANG" --quiet
}
...
[MODE] Student=$STUDENT_MODEL_FAMILY base=$STUDENT_BASE_MODEL_PATH adapter_only=$STUDENT_ADAPTER_ONLY
[MODE] Direct=$DIRECT_QA_MODE (default legacy_class_roi_obb)
[MODE] Eval=$EVAL_TOPOLOGY GPUs=$EVAL_GPUS base_port=$EVAL_BASE_PORT
...
[MODE] Socratic=$SOCRATIC_TOPOLOGY; shared=one 8091 TP${OMNI_TP_SIZE} endpoint on physical GPUs ${OMNI_VISIBLE_GPUS}/three separate message contexts; independent=8001/8002/8003
[MODE] prepare只创建独立配置、类别映射和运行签名。
EOF2
}

doctor() {
  echo "[DOCTOR] release=$RELEASE_REVISION"
  echo "[DOCTOR] script=$SCRIPT_PATH"
  echo "[DOCTOR] root=$RS"
  "$BOOTSTRAP_PY" "$RS/tools/verify_v123r3_install.py" --root "$RS"
  ensure_active_dataset
  if [[ ! -f "$ACTIVE_CFG" ]]; then
    echo "[DOCTOR] active settings不存在，先自动执行prepare: $ACTIVE_CFG"
    prepare
  else
    require_cfg
    "$BOOTSTRAP_PY" "$RS/tools/validate_active_taxonomy.py" \
...
data() {
  require_cfg
  cd "$RS"
  "$BOOTSTRAP_PY" main_layer/run.py data-full --settings "$ACTIVE_CFG" --lang "$RUN_LANG"
...
known_eval_keys=(rs_eot rs_eot_b1_direct rs-eot-b1-direct rs_eot_b2_socratic qwen3_omni_b0 qwen3_omni_b1_direct qwen3_omni_b2_socratic)
read_eval_arrays() {
...
stop_eval_replicas() (
  # 子Shell隔离清理变量，避免Bash动态作用域把调用者的模型key改写成B2。
  [[ -f "$ACTIVE_CFG" ]] || return 0
  read_eval_arrays
  local eval_cleanup_port eval_cleanup_key
  for eval_cleanup_port in "${EVAL_PORT_ARRAY[@]}"; do
    for eval_cleanup_key in "${known_eval_keys[@]}"; do
      bash "$RS/test_layer/stop_eval_model.sh" "$ACTIVE_CFG" "$eval_cleanup_key" "$eval_cleanup_port" >/dev/null 2>&1 || true
    done
  done
)
active_model_key() {
  local stage=$1
  require_cfg
  "$BOOTSTRAP_PY" - "$ACTIVE_CFG" "$stage" <<'PY'
import json,sys
s=json.load(open(sys.argv[1],encoding='utf-8'))
route=s.get('model_routing',{})
key=route.get({'b0':'b0_model_key','b1':'b1_model_key','b2':'b2_model_key'}[sys.argv[2]])
if not key: raise SystemExit(f"missing model route for {sys.argv[2]}")
print(key)
PY
}
adapter_only_active() {
...
import json,sys
s=json.load(open(sys.argv[1],encoding='utf-8'))
print('1' if bool(s.get('model_routing',{}).get('adapter_only')) else '0')
PY
}
...
m=models[key]; p=Path(str(m['path']))
print(f"[EVAL PREFLIGHT] key={key} served={m.get('served_name')} path={p} adapter={m.get('adapter_path')}")
if not (p/'config.json').is_file():
    raise SystemExit(f"[FAIL] base model config missing: {p/'config.json'}")
adapter=str(m.get('adapter_path') or '')
if adapter:
    a=Path(adapter)
    if not (a/'adapter_config.json').is_file():
        raise SystemExit(f"[FAIL] B1 adapter missing: {a/'adapter_config.json'}\n[HINT] first run b1-train successfully")
PY
}

start_eval_replicas() {
  local requested_eval_key=$1
  eval_model_preflight "$requested_eval_key"
  read_eval_arrays
  stop_eval_replicas
  stop_socratic_all
  local -a pids=()
  ...
  for ((i=0;i<${#EVAL_GPU_ARRAY[@]};i++)); do
    echo "[EVAL START] model=$requested_eval_key gpu=${EVAL_GPU_ARRAY[$i]} port=${EVAL_PORT_ARRAY[$i]}"
    bash "$RS/test_layer/start_eval_model.sh" "$ACTIVE_CFG" "$requested_eval_key" "${EVAL_GPU_ARRAY[$i]}" "${EVAL_PORT_ARRAY[$i]}" &
    pids+=("$!")
  done
...
run_eval_matrix() {
  ...
  require_legacy_or_explicit_json_eval
  read_eval_arrays
  if [[ "$EVAL_TOPOLOGY" != "single" && ${#EVAL_GPU_ARRAY[@]} -gt 1 ]]; then
    bash "$RS/main_layer/run_test_matrix_replicas.sh" "$ACTIVE_CFG" "$key" "$EVAL_URLS_CSV" fresh
  else
...
eval_route_check() {
  local requested_eval_key
  requested_eval_key=$(active_model_key b0)
  local before="$requested_eval_key"
  stop_eval_replicas
  [[ "$requested_eval_key" == "$before" ]] || fail "eval清理改写了模型key: before=$before after=$requested_eval_key"
  echo "[PASS] eval-route-check：B0=$requested_eval_key；清理未改写模型key；Eval=$EVAL_TOPOLOGY GPUs=$EVAL_GPUS。"
}

b0() {
  local key
  key=$(active_model_key b0)
  eval_model "$key"
  echo "[PASS] B0完成：model_key=$key。"
}
...
b1_train_only() {
  ...
  cd "$RS"
  "$BOOTSTRAP_PY" "$RS/tools/check_student_training_support.py" --settings "$ACTIVE_CFG"
  "$BOOTSTRAP_PY" main_layer/run.py train-b1-direct-standalone --settings "$ACTIVE_CFG"
  if [[ "$(adapter_only_active)" == "1" ]]; then
    echo "[B1] adapter-only route: skip merge into quantized base model."
  else
  ...
  "$BOOTSTRAP_PY" "$RS/tools/summarize_pipeline_v1_2_3.py" --settings "$ACTIVE_CFG" || true
  echo "[PASS] B1 Direct training completed."
}
b1_eval_only() {
  local key
  require_cfg
  "$BOOTSTRAP_PY" "$RS/main_layer/run.py" register-b1-direct-model --settings "$ACTIVE_CFG"
  key=$(active_model_key b1)
  eval_model "$key"
}
...
omni_preflight() {
  ...
  env CUDA_VISIBLE_DEVICES="$OMNI_VISIBLE_GPUS" \
...
socratic_audits() {
  local cfg=$1 mode=$2
  "$BOOTSTRAP_PY" "$RS/main_layer/run.py" inspect-api-truncation --settings "$cfg" --lang "$RUN_LANG" --mode "$mode"
  "$BOOTSTRAP_PY" "$RS/main_layer/run.py" check-debug-infra --settings "$cfg" --lang "$RUN_LANG"
  "$BOOTSTRAP_PY" "$RS/main_layer/run.py" check-debug --settings "$cfg" --lang "$RUN_LANG"
}
socratic_generate() {
  local cfg=$1 mode=$2 samples=$3
  "$BOOTSTRAP_PY" "$RS/main_layer/run.py" official-socratic-full --settings "$cfg" --lang "$RUN_LANG" train "$mode" "$samples" fresh
}
...
b2_train_only() {
  require_cfg
  if [[ "$STUDENT_MODEL_FAMILY" == "qwen3_omni" ]]; then
    fail "Qwen3-Omni B2 adapter-only training has not been enabled in this Direct-only package. This guard prevents the old RS-EoT merge route from being used accidentally."
  fi
  stop_eval_replicas; stop_socratic_all
  "$BOOTSTRAP_PY" "$RS/tools/audit_b1_b2_direct_pairing.py" --settings "$ACTIVE_CFG"
  archive_paths b2
...
b2_eval_only() {
  require_cfg
  [[ "$STUDENT_MODEL_FAMILY" != "qwen3_omni" ]] || fail "Qwen3-Omni B2 is intentionally blocked in this Direct-only package."
  "$BOOTSTRAP_PY" "$RS/main_layer/run.py" register-b2-model --settings "$ACTIVE_CFG"
  eval_model "$(active_model_key b2)"
}
...
safe_direct() { echo "[FLOW] safe-direct uses explicit model_routing B0/B1; RS-EoT registrations remain untouched; B2 is not requested."; prepare; data; b0; b1; echo "[PASS] safe-direct完成；未启动Socratic。"; }
...
基础：
  doctor                 验证r3实际安装位置、源码指纹、活动配置和类别路径
  eval-route-check       只读检查评估清理不会把B0/B1模型键改写成B2
  ...
  b0                     按 model_routing 评估当前学生基座（默认Qwen3-Omni）
  b1                     当前学生模型Direct LoRA；Qwen3-Omni为adapter-only，不merge AWQ
...
Socratic：
  ...
  socratic-one-debug     共享Omni：8091单端点、可配置TP、三独立messages，Debug40
...
case "${1:-}" in
  doctor) doctor ;; eval-route-check) eval_route_check ;; status) status ;; split-scenes) build_scene_disjoint ;; prepare) prepare ;; data|datafull) data ;; b0) b0 ;;
