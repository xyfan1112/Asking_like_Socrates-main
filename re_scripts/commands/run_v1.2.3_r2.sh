...
PY
  ...
  local verified_classes
  verified_classes=$(active_classes)
  [[ "$verified_classes" == "$active_classes" ]] || fail "prepare后类别路径不一致: generated=$active_classes verified=$verified_classes"
  echo "[TAXONOMY READY] classes_file=$verified_classes"
...
active_classes() {
  # Pure getter: stdout must contain exactly one path because callers use command substitution.
  # Do not call require_cfg/ensure_active_dataset here; their progress logs would corrupt the path.
  [[ -f "$ACTIVE_CFG" ]] || fail "active settings不存在: $ACTIVE_CFG。请先运行 prepare。"
...
try:
    ...
    raw=cfg['taxonomy']['classes_file']
except Exception as exc:
    print(f"[FAIL] 无法从active settings读取taxonomy.classes_file: {cfg_path}: {exc}", file=sys.stderr)
    raise SystemExit(2)
if not isinstance(raw,str) or not raw.strip():
    print(f"[FAIL] taxonomy.classes_file为空或不是字符串: {raw!r}", file=sys.stderr)
    raise SystemExit(2)
if '\n' in raw or '\r' in raw:
    print(f"[FAIL] taxonomy.classes_file含换行，配置已污染: {raw!r}", file=sys.stderr)
    raise SystemExit(2)
...
if not p.is_absolute():
    ...
    p=p.resolve()
if not p.is_file():
    print(f"[FAIL] active classes文件不存在: {p}", file=sys.stderr)
    raise SystemExit(2)
print(p)
PY
}
