...
"$SFT_PYTHON" "$ROOT/train_layer/01c_validate_d1_direct_only.py" --settings "$SETTINGS"
...
print(s['paths']['training_run_root']+'/configs/d1_direct_only_lora.yaml')
PY
)
[[ -f "$CFG" ]] || { echo "[FAIL] D1 config missing: $CFG" >&2; exit 2; }
