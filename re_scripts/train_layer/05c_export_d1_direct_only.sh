...
"$SFT_PYTHON" "$ROOT/train_layer/02c_generate_d1_direct_only_config.py" --settings "$SETTINGS"
...
print(t['d1_adapter_output'])
print(t['d1_merged_output'])
PY
)
...
ADAPTER=$(resolve_adapter "$ADAPTER_ROOT") || { echo "[FAIL] D1 adapter unavailable: $ADAPTER_ROOT" >&2; exit 2; }
BASE_CFG="$RUN/configs/d1_export.yaml"
RESOLVED="$RUN/configs/d1_export_resolved.yaml"
