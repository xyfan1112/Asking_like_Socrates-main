#!/usr/bin/env bash
# Generic N-replica single-GPU evaluation; URL count determines shard count.
set -uo pipefail
...
run_stage() {
  local name=$1; shift
  echo "[MATRIX REPLICAS] START $name"
  "$@"; local code=$?
  STAGE_NAMES+=("$name"); STAGE_CODES+=("$code")
  echo "[MATRIX REPLICAS] END $name exit=$code"
}
...
report={
  'schema_version':'test_matrix_v1_2_3_replicas',
  'model_key':sys.argv[2], 'run_policy':sys.argv[3],
  'topology':'N independent single-GPU vLLM replicas with stable SHA256 shards',
...
echo "[OK] replica matrix completed for $KEY"
