...
您可以通过移除 Dockerfile 中 `VOLUME [ "/home/yk/.cache/huggingface", "/app/shared_data", "/app/output" ]` 的注释来使用数据卷。

在构建 Docker 时使用参数 `-v ./hf_cache:/home/yk/.cache/huggingface` 来挂载数据卷。各个数据卷的含义表示如下。
