...
def resize_long_side_to_224(img: Image.Image):
    """
    将图像的长边等比例缩放到224，返回缩放后的图像和原始尺寸。
    
    参数:
        img (PIL.Image): 输入图像
    
    返回:
        resized_img (PIL.Image): 缩放后的图像
    ...
    if width >= height:
        scale = 224 / width
        new_size = (224, int(height * scale))
    else:
        scale = 224 / height
        new_size = (int(width * scale), 224)

    resized_img = img.resize(new_size, Image.BILINEAR)
    return resized_img


# 根据is_thumbnail修改图像尺寸
@dataclass
class RMMQwen2VLPlugin(Qwen2VLPlugin):
    def _regularize_images(self, images: list["ImageInput"], is_thumbnail: list[bool], **kwargs) -> dict[str, list["ImageObject"]]:
        r"""Regularize images to avoid error. Including reading and pre-processing."""
        results = []
        for i, image in enumerate(images):
            ...
            if is_thumbnail[i]:
                image = resize_long_side_to_224(image)
    ...
    def get_mm_inputs(
        ...
        """
        self._validate_input(processor, images, videos, audios)
        return self._get_mm_inputs(images, videos, audios, processor, batch_is_thumbnail)
    ...
    def process_messages(
        ...
        merge_length: int = getattr(image_processor, "merge_size") ** 2
        if self.expand_mm_tokens:
            mm_inputs = self._get_mm_inputs(images, videos, audios, processor, is_thumbnail)
            image_grid_thw = mm_inputs.get("image_grid_thw", [])
            video_grid_thw = mm_inputs.get("video_grid_thw", [])
        else:
            image_grid_thw = [None] * len(images)
